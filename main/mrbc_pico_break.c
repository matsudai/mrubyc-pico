/*! @file
  ...
  Break信号の検出はTinyUSBのコールバックで行い，mrbc_tickコールバックでメインコンテキストからリブートを実行する．
*/
...
/** @brief Break信号によるリブート機能の初期化
  ...
  mrbc_tickコールバックにbreak_checkを登録する．
*/
void mrbc_pico_break_init(struct VM* vm) {
  (void)vm;
  mrbc_set_tick_callback(break_check);
}
